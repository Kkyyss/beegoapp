...
export default class RoomStatusPage extends Component {
  ...
  componentDidMount() {
    ...
    function calculateResource(ds) {
      var result;

      campusDataSource.TypesOfRooms.forEach(function(obj, index) {
        var total = 0;
        var totalNotAvailable = 0;
        ds.map(function(dsObj) {
          // console.log(dsObj);
          if (obj === dsObj.TypesOfRooms) {
            total++;
            if (!dsObj.IsAvailable) {
              totalNotAvailable++;
            }
          }
        });
        campusDataSource.Total.push(total);
        campusDataSource.NotAvailable.push(totalNotAvailable);
        console.log(campusDataSource.Total, campusDataSource.NotAvailable);
      });
